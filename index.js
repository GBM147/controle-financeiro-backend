...
// Educação — vem ANTES do boleto pois UNICID é pago via boleto
} else if (descMinuscula.includes('unicid') || descMinuscula.includes('faculdade') ||
           descMinuscula.includes('universidade') || descMinuscula.includes('escola') ||
           descMinuscula.includes('mensalidade')) {
    categoria = 'Educação';

// Pagamento de Fatura de Cartão (identificado: PAGAMENTO CARTAO CREDITO BCE)
} else if (descMinuscula.includes('pagamento cartao') || descMinuscula.includes('cartao credito') ||
           descMinuscula.includes('fatura')) {
    categoria = 'Pagamento de Fatura';

// Pagamento de Boleto (outros que não são educação)
} else if (descMinuscula.includes('pagamento de boleto') || descMinuscula.includes('boleto')) {
    categoria = 'Pagamento de Boleto';

// Alimentação (identificado: Café da Graça, Gran Coffee, Café CAOC, SERVANO, PRC ALI)
} else if (descMinuscula.includes('cafe') || descMinuscula.includes('coffee') ||
           descMinuscula.includes('servano') || descMinuscula.includes('prc ali') ||
           descMinuscula.includes('ifood') || descMinuscula.includes('restaurante') ||
           descMinuscula.includes('burger') || descMinuscula.includes('mcdonald') ||
           descMinuscula.includes('lanche') || descMinuscula.includes('padaria')) {
    categoria = 'Alimentação';

// Transporte (identificado: Uber TRIP)
} else if (descMinuscula.includes('uber') || descMinuscula.includes('99app') ||
           descMinuscula.includes('combustivel') || descMinuscula.includes('posto') ||
           descMinuscula.includes('metro') || descMinuscula.includes('onibus')) {
    categoria = 'Transporte';

// Telecomunicações (identificado: UP MOBILE MANAGEMENT LTDA)
} else if (descMinuscula.includes('up mobile') || descMinuscula.includes('vivo') ||
           descMinuscula.includes('tim ') || descMinuscula.includes('claro') ||
           descMinuscula.includes('oi ') || descMinuscula.includes('net ')) {
    categoria = 'Telecomunicações / Internet';

// Igreja e Doações (identificado: Igreja Evangelica Familia)
} else if (descMinuscula.includes('igreja') || descMinuscula.includes('evangelica') ||
           descMinuscula.includes('pastoral') || descMinuscula.includes('doacao') ||
           descMinuscula.includes('caridade')) {
    categoria = 'Igreja / Doações';

// Entretenimento (identificado: BYTEDANCE BRASIL = TikTok)
} else if (descMinuscula.includes('bytedance') || descMinuscula.includes('netflix') ||
           descMinuscula.includes('spotify') || descMinuscula.includes('prime') ||
           descMinuscula.includes('youtube') || descMinuscula.includes('disney') ||
           descMinuscula.includes('tiktok') || descMinuscula.includes('globoplay')) {
    categoria = 'Entretenimento';

// Supermercado
} else if (descMinuscula.includes('mercado') || descMinuscula.includes('carrefour') ||
           descMinuscula.includes('atacadao') || descMinuscula.includes('assai') ||
           descMinuscula.includes('extra')) {
    categoria = 'Supermercado';

// Taxas Bancárias (identificado: JUROS, MULTA MORATORIA, IOF no seu extrato)
} else if (descMinuscula.includes('juros') || descMinuscula.includes('multa') ||
           descMinuscula.includes('iof') || descMinuscula.includes('mora') ||
           descMinuscula.includes('tarifa') || descMinuscula.includes('encargo')) {
    categoria = 'Taxas Bancárias';

// Crédito Liberado para PIX (banco adianta o valor via cartão)
} else if (descMinuscula.includes('credito liberado')) {
    categoria = 'Crédito Cartão';

// PIX entre pessoas (enviados e recebidos de pessoas físicas)
} else if (descMinuscula.includes('pix recebido') || descMinuscula.includes('pix enviado') ||
           descMinuscula.includes('pix open') || descMinuscula.includes('ted ') ||
           descMinuscula.includes('doc ')) {
    categoria = 'Transferência';
}
