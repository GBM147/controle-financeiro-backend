...
function detectarDestino(pergunta) {
    ...
    const regras = [
        ...
        { termos: ['educacao financeira', 'aprender sobre finanças', 'aprender a organizar dinheiro'], pagina: 'educacao-financeira.html', descricao: 'Para conteúdo educativo, use Educação financeira.' }
    ];
...
function criarPrompt(pagina, pergunta, historico) {
    const dados = PAGINAS[pagina];
    const mapaPaginas = Object.entries(PAGINAS)
        .map(([chave, valor]) => `${valor.titulo}: ${valor.url} — ${valor.contexto}`)
        .join('\n');
...
async function gerarResposta(pergunta, pagina, historico) {
    ...
    if (!genAI) {
        return {
            texto: 'Não consegui acessar a IA agora. Ainda posso indicar páginas conhecidas do GBM quando a pergunta corresponder a uma delas.',
            navegacao
        };
    }
    ...
    try {
        ...
        return {
            texto: navegacao
                ? `Não consegui consultar a IA agora, mas acredito que esta seja a página certa: ${navegacao.titulo}. ${navegacao.descricao}`
                : 'Não consegui responder essa dúvida agora. Não tenho informação suficiente para confirmar a função no GBM.',
            navegacao
        };
    }
}
    ...
    app.post('/assistente-ajuda', async (req, res) => {
        ...
        try {
            ...
            return res.json({
                ...
                navegacao: resultado.navegacao || null
            });
        } catch (erro) {
